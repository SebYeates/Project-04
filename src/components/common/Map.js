...
class Map extends React.Component {
  ...
  componentDidMount() {
    ...
    this.markers = this.props.courses.map(course => {
      const { lat, lng } = course.location
    ...
    if(this.props.type !== 'course'){
      ...
      const { lat, lng } = this.props.courses[0].location
      this.map.flyTo({
  ...
  generatePopups() {
    ...
    this.props.courses.map((course, index) => {
      ...
      const { location, name, image, _id } = course

      //add a popup
      // NOTE: view course Link needs to be updated for heroku
      ...
      if(this.props.type === 'course'){
        ...
        this.popup.setHTML(`
          ...
          <h4>${name}</h4>
          <a href="https://www.google.com/maps/dir/?api=1&origin=${lat},${lng}&destination=${location.lat},${location.lng}" target="_blank" > Directions </a>
          <a href="${process.env.PATH}/courses/${_id}">View Course</a>
          `)
      }
    })
  }
